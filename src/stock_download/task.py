...
def update_stock_json(code, field_name, data, append=False, unique_key="交易日期", save_to_quote_dir=False):
    ...
    """
    # 历史K线单独存储为CSV，不写入JSON文件
    if field_name == "history_kline":
        # 构造CSV文件路径
        csv_dir = PATH["history_dir"]
        os.makedirs(csv_dir, exist_ok=True)
        csv_path = os.path.join(csv_dir, f"{code}_history_kline.csv")
        mode = "append" if append else "cover"
        # 保存为CSV格式
        DataSaver.save(data, csv_path, format="csv", mode=mode, field_name=field_name, unique_key=unique_key)
        return
